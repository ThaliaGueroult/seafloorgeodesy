...
import pyproj
from navpy import ecef2ned, ned2ecef

def lonlathalt_to_ned(reference_lonlathalt, lonlathalt):
    x, y, z = navpy.lla2ecef(lonlathalt[:,1], lonlathalt[:,0], lonlathalt[:,2], latlon_unit='deg', alt_unit='m', model='wgs84')
    ned_coords = navpy.ecef2ned(x, y, z, reference_lonlathalt[1], reference_lonlathalt[0], reference_lonlathalt[2], latlon_unit='deg', alt_unit='m', model='wgs84')
    return ned_coords

def ned_to_lonlathalt(reference_lonlathalt, ned_coords):
    x, y, z = navpy.ned2ecef(ned_coords[:,1], ned_coords[:,0], ned_coords[:,2], reference_lonlathalt[1], reference_lonlathalt[0], reference_lonlathalt[2], latlon_unit='deg', alt_unit='m', model='wgs84')
    lat, lon, alt = navpy.ecef2lla(x, y, z, latlon_unit='deg', alt_unit='m', model='wgs84')
...
filtered_data = []
for datetimes, x, y, z, lon, lat, elev in all_data:
    mask = np.isin(datetimes, common_datetimes)
    filtered_data.append((datetimes[mask], x[mask], y[mask], z[mask], lon[mask], lat[mask], elev[mask]))

# Définir le point de référence sur la base des données de l'Antenne 3 au premier timestamp
reference_lonlathalt = (filtered_data[2][4][0], filtered_data[2][5][0], filtered_data[2][6][0])

# Convertir les coordonnées ECEF en NED relatives à la position de référence de l'Antenne 3 à l'index 0
filtered_data_ned = []
for data in filtered_data:
    datetimes, x, y, z, lon, lat, elev = data
    ecef_coords = np.column_stack((x, y, z))
    ned_coords = ecef2ned(ecef_coords, reference_lonlathalt[0], reference_lonlathalt[1], reference_lonlathalt[2])
    filtered_data_ned.append((datetimes, ned_coords[:, 0], ned_coords[:, 1], ned_coords[:, 2], lon, lat, elev))

# Maintenant pour la référence du repère body par rapport à l'Antenne 3
antenna_1_data = filtered_data_ned[0]
antenna_2_data = filtered_data_ned[1]
antenna_3_data = filtered_data_ned[2]
antenna_4_data = filtered_data_ned[3]
...
for t in range(len(common_datetimes)):
    ...
    relative_positions_body_frame.append([antenna_3_position_t + levier for levier in leviers_a3])

# Calculate rotation matrices
rotation_matrices = []
for t in range(len(common_datetimes)):
    A = np.array(relative_positions_body_frame[t])
    B = np.array([(data[1][t], data[2][t], data[3][t]) for data in filtered_data_ned])
...
positions_ecef = ned2ecef(
    positions_transducer,
    reference_lonlathalt[0],
    reference_lonlathalt[1],
    reference_lonlathalt[2]
)
x_transducer = positions_ecef[:, 0]
y_transducer = positions_ecef[:, 1]
z_transducer = positions_ecef[:, 2]
