...
data_unit1 = sio.loadmat('../../data/SwiftNav_Data/Unit1-camp_bis.mat')
data_unit2 = sio.loadmat('../../data/SwiftNav_Data/Unit2-camp_bis.mat')
data_unit3 = sio.loadmat('../../data/SwiftNav_Data/Unit3-camp_bis.mat')
data_unit4 = sio.loadmat('../../data/SwiftNav_Data/Unit4-camp_bis.mat')
#
#
def plot_antenna_data(unit_number, subplot_num, color):
    ...
    plt.plot(times, lat, label=f"Latitude Antenna {unit_number}", color=color)
    plt.xlabel('Time')
    plt.ylabel('Latitude')
    plt.legend()

    plt.subplot(3, 2, subplot_num + 2)
    plt.plot(times,lon, label=f"Longitude Antenna {unit_number}", color=color)
    plt.xlabel('Time')
    plt.ylabel('Longitude')
    plt.legend()

    plt.subplot(3, 2, subplot_num + 4)
    plt.plot(times, elev, label=f"Elevation Antenna {unit_number}", color=color)
    plt.xlabel('Time')
...
def get_elevation_and_time(unit_number, sampling_interval=20):
    data_unit = sio.loadmat(f'../../data/SwiftNav_Data/Unit{unit_number}-camp_bis.mat')
    elev = data_unit['elev'].flatten()
    times = data_unit['times'].flatten()

    # Sous-échantillonner les données
    indices = range(0, len(times), sampling_interval)
    times_sampled = times[indices]
    elev_sampled = elev[indices]
    mean = np.mean(elev)

    return times_sampled, elev_sampled, mean


# Obtenez les élévations et les temps pour chaque unité
times_1, elev_1, mean1 = get_elevation_and_time(1)
times_2, elev_2, mean2 = get_elevation_and_time(2)
times_3, elev_3, mean3 = get_elevation_and_time(3)
times_4, elev_4, mean4 = get_elevation_and_time(4)

print(mean1, mean2, mean3, mean4)

plt.figure(figsize=(10, 6))

# Tracer les élévations en fonction du temps pour chaque unité
plt.scatter(times_1[50:-50], elev_1[50:-50], label='Antenna 1', s=5)
plt.scatter(times_2[50:-50], elev_2[50:-50], label='Antenna 2', s=5)
plt.scatter(times_3[50:-50], elev_3[50:-50], label='Antenna 3', s=5)
plt.scatter(times_4[50:-50], elev_4[50:-50], label='Antenna 4', s=5)

# Mettre en place les labels et les titres
plt.xlabel('Time')
plt.ylabel('Elevation')
plt.title('Elevation vs. Time for All Antennas')
plt.legend()
plt.grid(True)
