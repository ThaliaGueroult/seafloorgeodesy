...
def dist_analytic(dz, beta):
    ...
    while alpha_step > 10e-8:
        ...
        for i, angle in enumerate(alpha):
            ...
            # # Créer un nouveau réseau z pour l'interpolation
            # z = np.linspace(0, np.max(z_1), int(1e6))
            #
            # # Utiliser np.interp pour interpoler les données
            # x = np.interp(z, z_1, x)
            # t = np.interp(z, z_1, t)
...
def table_angle(dz, step):
    list_beta = np.arange(10,90,step)
    print(list_beta)
    ...
    filename = f'castbermuda/data_{dz}_step_angle_{step}.mat'
    savemat(filename, data)

def plot_esv():

    # Numéros de fichiers de 5226 à 5249
    file_numbers = range(5208, 5230)
...
def plot_and_save_esv():
    ...
    for i, file_number in enumerate (file_numbers):
        # Nom du fichier
        file_name = f"castbermuda/data_{file_number}_step_angle_5.mat"
        ...
        new_angles = np.arange(10, 90, 0.01)
        ...
        new_file_name = f"castbermuda/mat_interp/data_{file_number}_step_angle_0_01.mat"
        sio.savemat(new_file_name, {'elevation angle (deg)': new_angles, 'esv (m/s)': new_esv})

        plt.scatter(new_angles, new_esv, label=labels[i], color=colors[i])
